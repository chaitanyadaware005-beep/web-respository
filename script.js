Const GENRES = [ 
'abstract','geometric','landscape','portrait','cosmic','fluid']; 
const ARTISTS = [ 
  'Elena Voss','Marco Delgado','Yuki Tanaka','Priya Nair','Axel 
Brandt', 
  'Selin Çelik','Omar Hassan','Lena Möller','Rin Sato','Aiko 
Fujita', 
  'Dario Ferretti','Mia Kovacs','Zen Li','Asha Osei','Felix 
Wagner', 
]; 
const TITLES = [ 
  'Cascade of Light','Void Within','Amber Horizon','Silent 
Frequency', 
  'Neon Dream','Fractal Mind','Echo Chamber','Twilight Prism', 
  'Burning Archive','Deep Structure','Soft Collapse','Quantum 
Bloom', 
  'Edge of Memory','Glitch Pastoral','Indigo Pulse','Dark 
Matter', 
  'Chromatic Drift','Signal & Noise','Rust Elegy','Aurora 
Protocol', 
  'Molten Core','Fragile Lattice','Spectral Body','Zero 
Gravity', 
  'Morning Decay','Tidal Algorithm','Iron Sunrise','Parallel 
Garden', 
  'Static Garden','Digital Palimpsest','Hollow Star','Phase 
Shift', 
  'Luminous Void','Copper Sky','Geometric Soul','Neon Relic', 
  'Thermal Bloom','Wave Function','Distant Shore','Phantom 
Signal', 
  'Crystal Lens','Urban Mirage','Soft Entropy','Black Ice', 
  'Resonant Field','Color Theory','Spectral Gate','Folded 
Light', 
  'Cosmic Shore','Emulsive Dream','Blur Study','Interference 
Pattern', 
  'Liminal Space','Neural Garden','Dusk Erosion','Arc of 
Silence', 
]; 
 
const YEARS  = ['2021','2022','2023','2024','2025','2026']; 
const MEDIA  = ['Generative Code','Canvas API','Shader 
Art','Data Painting','Pixel Synthesis','Algorithmic Form']; 
...
drawFns.push(function(ctx, w, h, s, pal) { 
  ...
  for (let i = 0; i < 60; i++) { 
    const x = r() * w, y = r() * h, radius = r() * w * 0.35 + 
10; 
...
drawFns.push(function(ctx, w, h, s, pal) { 
  ...
  for (let m = 0; m < 3; m++) { 
    ...
    for (let x = 0; x <= w; x += 15) { 
      const y = h * (0.4 + m * 0.12) + Math.sin(x * 0.02 + m) * 
40 * r() - r() * 30; 
      ctx.lineTo(x, y); 
    } 
  ...
  for (let i = 0; i < 40; i++) { 
    ...
    ctx.beginPath(); 
    ctx.arc(r() * w, r() * h * 0.45, r() * 2 + 0.5, 0, Math.PI * 
2); 
    ctx.fill(); 
  } 
  ctx.globalAlpha = 1; 
...
0.6); 
  ...
  ctx.beginPath(); 
  ctx.ellipse(cx, cy - 10, w * 0.22, h * 0.28, 0, 0, Math.PI * 
2); 
  ctx.fill(); 
  for (let i of [-1, 1]) { 
    ctx.fillStyle = pal[2]; 
    ctx.beginPath(); 
    ctx.ellipse(cx + i * w * 0.08, cy - h * 0.05, w * 0.04, h * 
0.025, 0, 0, Math.PI * 2); 
    ctx.fill(); 
    ctx.fillStyle = '#0a0a0f'; 
    ctx.beginPath(); 
    ctx.arc(cx + i * w * 0.08, cy - h * 0.05, w * 0.02, 0, 
Math.PI * 2); 
    ctx.fill(); 
  } 
...
drawFns.push(function(ctx, w, h, s, pal) { 
  ...
  for (let i = 0; i < 4; i++) { 
    const grd = ctx.createRadialGradient(r() * w, r() * h, 0, 
r() * w, r() * h, w * 0.4); 
  ...
  const grd2 = ctx.createRadialGradient(w / 2, h / 2, 0, w / 2, 
h / 2, w * 0.18); 
...
drawFns.push(function(ctx, w, h, s, pal) { 
  ...
  for (let py = 0; py < h; py++) { 
    for (let px = 0; px < w; px++) { 
      ...
      const v = Math.sin(px * freq + py * 0.02) * Math.cos(py * 
freq2 + px * 0.015); 
      const norm = (v + 1) * 0.5; 
...
TITLES.length ? ' ' + (Math.floor(idx / TITLES.length) + 1) : 
''); 
  ...
  return { idx, genre, title, artist, year, medium, palIdx, 
fnIdx }; 
} 
const ARTWORKS = Array.from({ length: 150 }, (_, i) => 
makeArtwork(i)); 
...
function renderCanvas(canvas, art, w, h) { 
  ...
  drawFns[art.fnIdx](ctx, w, h, art.idx * 1337 + 42, 
PALETTES[art.palIdx]); 
} 
...
[...ARTWORKS, ...ARTWORKS].forEach(art => 
walkRow.appendChild(makeCard(art))); 
...
b.classList.remove('active')); 
  ...
  document.querySelectorAll('.artwork-card').forEach(card => { 
    card.style.display = (genre === 'all' || card.dataset.genre 
=== genre) ? '' : 'none'; 
  }); 
}); 
...
lb.addEventListener('click', e => { if (e.target === lb) 
lb.classList.remove('open'); }); 
document.addEventListener('keydown', e => { if (e.key === 
'Escape') lb.classList.remove('open'); });
